...
import static me.StevenLawson.TotalFreedomMod.World.TFM_DonatorWorld.TimeOfDay.values;
...
public final class TFM_DonatorWorld extends TFM_CustomWorld
{
    ...
    private static final String WORLD_NAME = "donorworld";
    //
    ...
    protected World generateWorld()
    {
        ...
        welcomeSign.setLine(0, ChatColor.GREEN + "DonorWorld");
    ...
    {
        if (guest == supervisor || TFM_DonatorList.isDonator(guest))
        {
            return false;
        }

        if (TFM_DonatorList.isDonatorPlus(supervisor))
        {
    ...
    public boolean canAccessWorld(final Player player)
    {
        ...
        if (cached == null)
        {
            ...
            if (!canAccess)
            {
                ...
                canAccess = supervisor != null && supervisor.isOnline() && TFM_DonatorList.isDonatorPlus(supervisor);
                if (!canAccess)
                {
